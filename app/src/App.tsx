          ...
          {SECTIONS.map((s) => (
            ...
            >
              ...
              {/* Общий зачёт — сразу за последним разделом, где тратятся деньги
                  ...
                  ⛔ ВНУТРИ секции, а не рядом с ней. Стоя отдельным элементом
                  общей сетки, зачёт отделялся от итогов теми же 40 px, что
                  разделы друг от друга, — и читался как ещё один раздел без
                  названия. Заказчик 09.08.2026: «итог по закупке и взаиморасчёты,
                  и вот всё, что прописано, должно вместе быть — это как будто
                  единая слитная информация». Теперь между итогами и зачётом
                  те же 16 px, что между блоками внутри раздела, и живёт он
                  под липкой полосой «Расходы».

                  Экземпляр один (У-53), арифметика прежняя — lib/settle.ts. */}
              {s.id === LAST_MONEY_SECTION && (
                <div className="mt-4">
