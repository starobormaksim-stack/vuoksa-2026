      ...
      {TILES.map((t) => (
        <div key={t.key} className="row-span-2 grid min-w-0 grid-rows-subgrid gap-0">
          ...
          {/* Число прижато к низу своей клетки: подпись сверху может занять одну
              ...
              Само число — кнопка к своей строке в «Итогах поездки» («Расходы»),
          ...
          <button
            type="button"
            onClick={() => {
              /* Подраздел «Итоги поездки» по умолчанию свёрнут (foldpref.ts) —
                 без заявки прыжок пришёл бы в пустоту. Раздел теперь «Расходы»:
                 «Дороги» на листе больше нет (09.08.2026). */
              requestUnfold('buy', 'sum-' + t.slot)
              jumpToItem('buy', 'sum-' + t.slot)
            }}
