...
import { breakdownOf, rankedPeople, missingLineOf } from '@/lib/gearx'
 ...
 *
 * Процентов и полос здесь нет: заказчик спросил, зачем ему доля, — ему нужно,
 * сколько позиций не собрано и какие именно. Строка их и называет.
 *
          ...
          return (
            ...
            >
              <button
                type="button"
                onClick={() => setWho(p.id)}
                aria-label={`${p.name}: ${missingLineOf(b)}. Открыть разбор`}
