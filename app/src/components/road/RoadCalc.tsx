...
function Matrix({
  ...
  /* ── Аренда ──
     Заказчик 08.08.2026: «Аренда уходит отдельно… не как сейчас „аренда
     и парковка“. Аренда может быть и в сутках, и в часах, и в минутах, это
     может быть место проживания, это может быть, кстати, парковка. Пункт
     будет называться аренда», а парковка «вписывается просто внутри неё».
  ...
  const totals: { slot: 0 | 1 | 2 | 3; label: string; text: string; sum: number }[] = [
    ...
    },
    {
      slot: 1,
      label: 'Продукты',
      text: 'То, что помечено «купить» в разделе «Расходы»',
      sum: c.buy,
    },
