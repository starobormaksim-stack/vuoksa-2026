 ...
 *
 * ⛔ Ленты точек маршрута здесь БОЛЬШЕ НЕТ, и вернуть её нельзя. Заказчик
 * 06.08.2026, дважды и дословно: «разметка маршрута в виде списка не нужна
 * вообще — прям на карте, чтобы это всё обозначалось и было видно» и потом
 * «Да, она не нужна вообще. Просто список точек на карте». Всё, что на ленте
 * висело, переехало в карточку метки на карте (`map/MapPointCard.tsx`):
 * адрес стал правиться, «пройдено» и «кто едет» встали своими органами,
 * метка этапа, чем добираемся, расстояние и координаты — в подробностях.
 * Точки без координат метки на карте не имеют вовсе — ими занят мастер
 * «Разметить маршрут» под самой картой.
 *
