 ...
 *
 * `text` — одна фраза о том, чего не хватает и что от этого меняется. Объяснять
 * ею устройство интерфейса («нажмите кнопку ниже», «тап по строке открывает…»)
 * не нужно: кнопка стоит тут же и подписана глаголом.
 */
interface Props {
  icon: LucideIcon
  title: string
  ...
  return (
    <div className="flex flex-col items-center gap-4 px-6 py-10 text-center">
      ...
      <div>
        ...
        <p className="mx-auto mt-1 max-w-64 text-note text-muted text-balance">{text}</p>
      </div>
      {action && (
