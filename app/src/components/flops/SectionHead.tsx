    ...
    <div className="mb-3">
        ...
        {canEdit ? (
          <h2 className="min-w-0 flex-1">
            <button
              ...
              className="editable -mx-2 flex min-h-11 max-w-full items-center rounded-xl px-2 text-left text-2xl font-[700] text-ink transition-colors hover:bg-zebra"
            >
              <span className="truncate">{шапка}</span>
            </button>
          </h2>
        ) : (
          <h2 className="min-w-0 flex-1 text-2xl font-[700] text-ink">{шапка}</h2>
        )}
        {onHelp && (
          <button
            type="button"
      ...
      {canEdit ? (
        <button
          type="button"
          onClick={() => setEdit('sub')}
          className="editable -mx-2 mt-0.5 block min-h-8 max-w-full rounded-xl px-2 text-left text-[13px] text-muted transition-colors hover:bg-zebra"
        >
          {подпись || <span className="opacity-70">Подпись раздела</span>}
        </button>
      ) : подпись ? (
        <p className="mt-0.5 text-[13px] text-muted">{подпись}</p>
      ) : null}
      {children}
