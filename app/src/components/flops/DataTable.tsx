...
export function DataTable({ cols, label, sync, children, className }: TableProps) {
  ...
  return (
    <div ref={attach} onScroll={onScroll} className={cn('overflow-x-auto', className)}>
      <div
        role="grid"
        aria-label={label}
        /* `w-max` обязателен вместе с `min-w-full`: без него сетка сжимается
           до ширины экрана, и при прокрутке вбок зебра строк и липкая граница
           обрываются на середине — дальше идёт голый фон страницы. */
