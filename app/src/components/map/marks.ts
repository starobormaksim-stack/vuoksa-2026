...
/**
 * Тон единицы техники по её месту в списке `S.transport`. Техники может быть
 * больше, чем тонов, — тогда тона идут по кругу: это лучше, чем изобретать
 * новый цвет мимо бренда.
 */
export function toneAt(index: number): MapTone {
  if (index < 0) return COMMON_TONE
  const rest = MAP_TONES.length - 1
  return MAP_TONES[1 + (index % rest)]
}
