 ...
 *
 * Порядок точек внутри нитки задаёт `ord` — поле документа, а НЕ место точки
 * в массиве. Нитки идут в порядке `S.transport`, общая — первой: так тон каждой
 * техники не зависит от того, какую точку куда переставили.
 *
 * ─── ⛔ Почему именно `ord`, а не порядок массива ───
 * 08.08.2026 у «Chevrolet Aveo» стояло 742 км там, где по точкам выходило 456.
 * Причина: слияние (`pick` в lib/merge.ts) кладёт ЧУЖУЮ новую точку в КОНЕЦ
 * массива — сравнивать ей нечего, `ord` при этом приезжает свой. На той машине,
 * куда точка приехала, нитка шла Кронштадт → Приозерское → Приозерск →
 * Сестрорецк вместо Кронштадт → Сестрорецк → Приозерское → Приозерск, и расчёт
 * по карте честно намерил обратный конец: 49 + 51 + 116 + 155 = 371 км вместо
 * 49 + 24 + 39 + 116 = 228. Числа участков (`route[].leg`) записаны в документ
 * БЕЗ порядка, в котором их считали, — значит порядок обязан быть один
 * на все машины, и он записан в `ord` (по нему же идёт выгрузка, lib/export.ts).
 *
 * Сортировка устойчивая: у точек с одинаковым `ord` (старые документы, где поля
 * ещё не было) сохраняется прежний порядок массива.
 */
export function threads(points: RoutePoint[], transports: Transport[]): Thread[] {
  const order = transports.map((t) => t.i)
  const byOrd = points
    .map((p, idx) => ({ p, idx }))
    .sort((a, b) => (a.p.ord || 0) - (b.p.ord || 0) || a.idx - b.idx)
    .map((x) => x.p)
  ...
  const common = byOrd.filter((p) => !p.tr || !order.includes(p.tr))
  const out: Thread[] = [{ tr: '', tone: COMMON_TONE, leg: null, points: common }]
  transports.forEach((t, idx) => {
    const own = byOrd.filter((p) => p.tr === t.i)
