...
export function OsmRouteMap({
  ...
  /* ⛔ `tone` и `color` в слепке обязательны. Без них смена цвета ветки
     не перерисовывала карту вовсе: слой пересобирался только когда менялись
     точки или способ передвижения, а цвет ждал случайного повода. Человек
     выбирал цвет, полоса веток его показывала, линия оставалась прежней —
     ровно та беда, ради которой писан урок У-123, только на шаг ниже.
     Замерено 07.08.2026: свой цвет `#ff00aa` лёг в документ, а `stroke`
     линии остался `#C2410C`. */
  const sig =
