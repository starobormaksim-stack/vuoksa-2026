...
function PresenceChip({ people, compact }: { people: Person[]; compact?: boolean }) {
  ...
  /* Не узнали — это тоже сказано словами, молчание читается как «сервис сломан».
     ⚠️ Одним словом в полосе шапки объяснить нельзя, а длинной строке здесь
     не место (У-11: она съела названия разделов). Поэтому в полосе — короткое
     «не узнан», а что делать — наведением и вслух. Кто это читает: человек,
     открывший скачанную копию чужого листа, и тот, кто пришёл по ссылке
     без ключа. */
  if (!me)
    return (
      <span
        className="text-micro whitespace-nowrap text-muted"
        role="status"
        title="Лист не узнал вас. Откройте свою личную ссылку целиком, вместе с ключом"
        aria-label="Лист не узнал вас. Откройте свою личную ссылку целиком, вместе с ключом"
      >
        не узнан
      </span>
    )
