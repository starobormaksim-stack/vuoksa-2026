 ...
 *
 * ─── Где блок стоит ───
 * В САМОМ КОНЦЕ листа, после всех разделов (App.tsx). Путь у него длинный:
 * из «Дороги» в «Закупку» 05.08.2026 («всё должно быть в одном разделе, внизу
 * должны быть итоги»), из «Закупки» в конец листа 08.08.2026 — «Почему у тебя
 * взаиморасчёты после еды, после закупки идут, а отдельно расчёты по
 * логистике? У тебя всё в конце должно считаться». В конце «Закупки» зачёт
 * стоял РАНЬШЕ «Дороги», хотя топливо и аренду из неё уже учитывал.
 * ⛔ Экземпляр по-прежнему один: один блок в одном месте (У-53).
 *
...
/**
 * Столбцы: человек · уплачено · доля · баланс. Тянутся все — иначе имя занимало
 * 823 px из 1215, а три числа стояли в крайнем правом углу (та же жалоба
 * заказчика 05.08.2026, что и по расчёту дороги).
 */
const COLS = 'minmax(9rem,1.4fr) minmax(8rem,1fr) minmax(8rem,1fr) minmax(8.5rem,1fr)'
  ...
  return (
    ...
    <section className="overflow-clip rounded-xl border border-line bg-surface shadow-sm">
      <div className="border-b border-line px-4 py-3">
        ...
        {/* Одна строка вместо двух прежних абзацев: заказчик 05.08.2026 —
            «гигантское количество текста… писать целые статьи о том, как
            пользоваться конкретным элементом, это слишком». Правило расчёта
            здесь критическое (постулат 5), а инструкция — нет. */}
        <p className="mt-0.5 text-note leading-snug text-muted">
          Выложенное минус своя доля. Считается по всем тратам поездки
        </p>
      </div>
      {children}
    </section>
