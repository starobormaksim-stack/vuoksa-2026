 ...
 * 04.08.2026, вечер: заказчик попросил эмблему ещё на 20 % крупнее (1,4 → 1,68).
 * При `height` 26–28 (мобильная и десктопная шапки) эмблема выходит 44–47 px,
 * слово остаётся прежних 21–23 px. Вызывающий код менять не пришлось:
 * пропорция живёт здесь.
 *
 * Полоса шапки — 56 px на мобильном и 64 px на десктопе, так что 47 px эмблемы
 * встают внутрь: на мобильном остаётся по 6 px воздуха сверху и снизу, на десктопе
 * по 8. Дальше растить нельзя — эмблема упрётся в края полосы.
 */
const EMBLEM_RATIO = 1.68
/** Кегль слова от `height` — то же число, что было до правки: слово не растёт. */
const WORD_RATIO = 0.82

/**
 * Просвет между эмблемой и словом, долей от эмблемы (правка 04.08.2026, вечер).
 *
 * Заказчик: «увеличить немного расстояние от логотипа до слова… чтобы это
 * премиально выглядело в плане расстояния, чтобы принадлежность друг другу
 * сохранялась, при этом не было слишком плотно».
 *
 * 0,26 от эмблемы даёт 11–12 px при эмблеме 44–47 px — это примерно половина
 * кегля слова. Меньше половины читается как «слиплось», больше кегля — как два
 * отдельных предмета рядом. Просвет считается от ЭМБЛЕМЫ, а не от высоты слова:
 * растёт эмблема — растёт и воздух вокруг неё.
 */
...
export function Logo({ height = 28 }: { height?: number }) {
  ...
  return (
    <span
      ...
      style={{ height: emblem, gap: Math.round(emblem * GAP_RATIO) }}
      role="img"
      aria-label={BRAND}
    >
