...
function sheetRoad(S: State): Sheet {
  ...
  const rows: Row[] = [
    /* Ключ подраздела, а не раздела: «Дороги» на листе больше нет, логистика
       стала подразделом «Расходов», а своё название заказчика по-прежнему
       лежит в `S.secTitles.log` (`lib/sectitles.ts`, У-160). */
    [title(titleOf(S, 'log', 'Логистика'))],
