 ...
 *
 * ─── Модель прав НЕ меняется ───
 * Вход почтой ничего не отнимает и ничего не выдаёт: личные ссылки `?u=…&k=…` работают
 * как работали. Вход — дополнительное подтверждение того, что за документом владелец.
 */

import { SB, setAuthToken } from './supabase.ts'
