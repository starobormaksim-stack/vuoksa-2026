...
export interface Transport extends SpendShare {
  ...
  fuel: string
  /**
   * ─── Своя цена литра у этой единицы техники (заказчик 09.08.2026) ───
   * «Здесь каждая машина может заправиться на разной заправке, а соответственно
   * цена может быть разная. По умолчанию сейчас можно указать одинаковую».
   *
   * ⛔ Поле добавлено АДДИТИВНО и необязательно: пусто или ноль — берётся цена
   * ВИДА топлива из `S.fuelPrices` (`fuelPriceFor` в `lib/calc.ts`), то есть
   * ни один заведённый до этой правки документ в деньгах не сдвинулся.
   * Цена вида при этом остаётся общей и правится там же, где стояла, —
   * в строке «Цена АИ-95» над техникой.
   */
  fuelPr?: number
  rate: number
  rateU: RateUnitId
...
export interface Rent extends SpendShare {
  ...
  price: number
  /**
   * Цена по факту, за ту же единицу. Форма та же, что у покупки (`Buy.prf`):
   * в расчёте приоритетнее `price`, если больше нуля (постулат 3.5 — форма
   * у списков одна). Заказчик 09.08.2026 на вопрос «у аренды тоже две цены?» —
   * «да, план и факт». Поле необязательное: пусто — считается по `price`,
   * то есть контрольная цифра аренды (15 000 ₽) не двигается.
   */
  priceF?: number
  unit: string
  qty: number
