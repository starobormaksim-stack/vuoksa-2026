...
export interface Rent extends SpendShare {
  ...
  count: number
  /**
   * ─── Поля ночёвки (заказчик 09.08.2026) ───
   * «У „Проживания“ нужны все поля, и пустыми: адрес места, кто бронирует,
   * кто оплачивает, ссылка на бронь». «Кто оплачивает» — это уже `payer`
   * из `SpendShare`, второго поля под то же значение не заводится (У-58).
   *
   * ⛔ Все три необязательны и пусты по умолчанию: строка аренды, заведённая
   * до этой правки, ничего не теряет и в деньгах не двигается. Показываются
   * они у ЛЮБОЙ аренды, а не только у ночёвки: категория строки меняется
   * на месте, и поле, спрятанное по категории, пропало бы вместе с данными
   * (постулат 4).
   */
  /** адрес места словами; в будущем подтягивается с карты по выбранной точке */
  addr?: string
  /** id человека из S.people, который бронирует */
  book?: string
  /** ссылка на бронь */
  url?: string
  calcT: string
...
/**
 * S.canRows[] — строки блока «Топливо в канистрах», привязаны к fuelPrices
 * по `fuel`.
 *
 * ─── Своя расчётная строка, а не только «сколько нужно канистр» (09.08.2026) ───
 * Заказчик: «там будет фиксироваться стоимость… вбивается информация, сколько
 * они возьмут дополнительно бензина в канистрах: одну канистру, две.
 * Добавляется вид топлива».
 *
 * То есть строка отвечает на свой вопрос: сколько топлива берём В КАНИСТРАХ
 * СВЕРХ расчётного расхода и сколько это стоит. Посчитанное «сколько канистр
 * нужно, чтобы увезти топливо техники с отметкой „везём с собой“» (`cans()`
 * в `lib/calc.ts`, контрольная цифра «2 канистры») никуда не девается: оно
 * стоит отдельной посчитанной строкой в том же блоке и деньгами не становится —
 * это топливо уже посчитано в строке своей техники.
 *
 * ⛔ Поля `cans`, `price`, `payer` и `sp` добавлены АДДИТИВНО и необязательны:
 * пока канистр не вписано, стоимость строки равна нулю, и ни один заведённый
 * до этой правки документ в деньгах не сдвинулся (контрольные суммы).
 */
export interface CanRow extends SpendShare {
  i: string
  fuel: string
  t: string
  c: string
  /** сколько канистр берём сверх расчёта; пусто или ноль — не берём */
  cans?: number
  /** цена литра у этой строки; пусто или ноль — цена вида из S.fuelPrices */
  price?: number
  ord: number
  ua: number
}
