...
/**
 * Что может уровень — ОДНОЙ короткой строкой.
 *
 * Было четыре строки на роль плюс столько же «чего не может»: до семи пунктов
 * на пол-экрана. Заказчик 05.08.2026: «у тебя просто гигантское количество текста…
 * писать целые статьи о том, как пользоваться каким-то конкретным элементом, это
 * слишком» (урок У-84). Роль выбирается по образцу канала в Телеграме: три строки,
 * у каждой одна подпись, отметка стоит у выбранной.
 *
 * ⛔ Строка описывает роль, а не жест (постулат 7): «правит списки, деньги, маршрут»,
 * а не «нажмите, чтобы сменить».
 */
export function permShort(perm: Perm): string {
  if (perm === 'chief') return 'Всё, включая удаление'
  if (perm === 'editor') return 'Правит списки, деньги, маршрут'
  return 'Отмечает своё'
}

/** Три уровня в порядке убывания полномочий — для выбора роли на месте. */
export const PERM_ORDER: Perm[] = ['chief', 'editor', 'member']
...
export function makePerms(S: State, auth: Auth | null, stale = false): Perms {
  ...
  /* Забрать себе копию листа — Excel или офлайн-файл — может каждый из команды.
     Требование заказчика 05.08.2026, дословно: «имеется эта возможность у каждого
     из участников команды: что в Excel выгрузить себе, что офлайн-версию сохранить
     себе». Это выгрузка того, что человек и так видит на экране, а не правка.
     ⚠️ Личные ключи ЧУЖИХ людей в копию не уезжают: `saveOfflineCopy` вычищает их
     всем, кроме владельца (lib/offline.ts). Иначе участник, открыв скачанный файл
     в блокноте, получил бы ключ владельца — это ровно урок У-65. */
  const canSaveFile = () => true
